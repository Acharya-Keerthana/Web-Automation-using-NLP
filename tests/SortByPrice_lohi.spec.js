...
test("User able to sort products by price", async ({ page }) => {
  ...
  const sortedPrices = prices.sort((a, b) => {
    ...
    if (a === null) return 1 
    if (b === null) return -1 
    return a - b  //Nếu trả về âm thì a sẽ đứng trước b, nếu trả về dương thì b sẽ đứng trước a
  }) 
  /*
  ...
  */
  ...
  if (JSON.stringify(prices) === JSON.stringify(sortedPrices)) {
    console.log("Products are sorted by price from low to high") 
  } else {
    console.error(
      "Products are not sorted correctly by price from low to high"
    ) 
  }
